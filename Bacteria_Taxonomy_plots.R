...
#-- Removed all archea and Bacteira having less than 5 counts
#p__Armatimonadetes, p__Chlamydiae, p__Deferribacteres,p__Gemmatimonadetes
#p__LD1, #p__OD1, p__SR1,4  #p__WPS-2, #p__[Thermi]
taxonomy <- read.csv("../16S_analysis/16S_taxonomy_formatted.tsv", sep = "\t", row.names = 1)
...
MyPalette <- c("forestgreen", "#5DD0B9", "darkgoldenrod3", "chartreuse1", 
               ...
               "#de77ae", "gray93", "#6a3d9a", "darkgray")

jpeg("16S_Phylum.jpg", height = 10, width = 12, units = 'in', res = 600)
...
asv_table_in <- read.csv("../16S_analysis/featureTable_forCumulativePhylaPlot.txt", sep = "\t", row.names = 1)
asv_table_in_t <- data.frame(t(asv_table_in))
#ab <- table(unlist(asv_table_in_t))
...
jpeg("16S_Phylum_Cumulative.jpg", height = 8, width = 5, units = 'in', res = 600)
p <- plot_bar(ps_normalized, fill = "Phylum", facet_grid=~Family) + scale_fill_manual(values=MyPalette)
## add facets
...
source('~/Work/Project_16S_ITS/Combined/Bubble_16S.R')
